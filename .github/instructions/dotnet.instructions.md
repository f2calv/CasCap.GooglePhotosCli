...
- Keep Debug and Release variants named `CasCap.GooglePhotosCli.Debug.slnx` and `CasCap.GooglePhotosCli.Release.slnx`.
- The Debug solution uses local `ProjectReference` items for the CasCap.Api.GooglePhotos and CasCap.Common projects; the Release solution uses published `PackageReference` items.
